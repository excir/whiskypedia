    ...
    actions: {
        async fetchLibraries() {
            try {
                ...
                const libraries: Library[] = Array.isArray(response.data) ? response.data : [];
        ...
        async updateLibrary(library: Library) {
            try {
                const response = await axios.put(`${API_BASE}/library/${library.id}`, library);
        ...
        merge(library: Library, action: 'add' | 'delete' | 'update') {
            const libraryMap = this.libraryMap.find(libMap => libMap.name === library.name);
            if (action === 'add') {
                if (libraryMap) {
                    libraryMap.libraries.push(library);
                } else {
                ...
                if (libraryMap) {
                    libraryMap.libraries = libraryMap.libraries.filter(lib => lib.id !== library.id);
                    if (libraryMap.libraries.length === 0) {
                        this.libraryMap = this.libraryMap.filter(libMap => libMap.name !== library.name);
                    }
                }
            } else if (action === 'update') {
                if (libraryMap) {
                    const index = libraryMap.libraries.findIndex(lib => lib.id === library.id);
                    if (index !== -1) {
